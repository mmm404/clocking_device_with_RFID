...
unsigned long menu() {
 ...
 // tft.drawRoundRect(0,0,380,50,10,WHITE);
  ...
  tft.setCursor(60, 150);
  tft.println("3. Enroll/Flush members");
...
bool storeUid(String rfidUid) {
  ...
  if (availableAddr != -1) {
    ...
    if (rfidUid.length() <= MAX_UID_LENGTH) {
      // Write UID to EEPROM
      ind = getIndex();
      writeStringToEEPROM(availableAddr, "index:" + String(ind) + ",UID:" + rfidUid, MAX_UID_LENGTH);
      //writeStringToEEPROM(availableAddr, {"index":ind,"UID":rfidUid}, MAX_UID_LENGTH);
      Serial.print("Entry Stored successfully with index :  "); // Entry stored successfully
      Serial.print(ind);
      return true;
    } else {
...
trackUID(jObj){
  StaticJsonDocument<192> UID;
  DeserializationError error = deserializeJson(UID, jObj);
  ...
  const char* indx = UID["index"];
  indx = String(indx);
  const char* unique_UID = UID["UID"];
  unique_UID = String(unique_UID);
  return {indx,unique_UID};

}

void scanEEPROM(entity){
  int entity_present = 0;
  for (int i = 0; i < MAX_ENTRIES; i++) {
    int addr = i * MAX_UID_LENGTH;
    if (!isMemoryBlockEmpty(addr, MAX_UID_LENGTH)) {
      String eep_rfidUid = readStringFromEEPROM(addr, MAX_UID_LENGTH);
      Serial.print(rfidUid);
      StaticJsonDocument<192> UID;
      DeserializationError error = deserializeJson(UID, jObj);
    ...
    const char* eep_indx = UID["index"];
    indx = String(eep_indx);
    const char* eep_unique_UID = UID["UID"];
    unique_UID = String(eep_unique_UID);
    
    if (entity == eep_indx){
      entity_present = addr;
    }

    else if(entity  == eep_unique_UID){
      entity_present = addr;
    }
  ...
  if(entity_present != 0){
    EEPROM.write(entity_present, 0xFF);
    displayLoadingAnimation()
    detailsDeleted();
    delay(1000);
    enrollNew();
...
void enrollNew(){
  ...
  tft.setCursor(60, 150);
  tft.println("3. Delete a Member");
  tft.setCursor(60, 200);
  tft.println("4. Go Back ");
}
