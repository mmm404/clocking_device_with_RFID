...
#define MAX_ENTRIES 200      // Maximum number of entries to store
...
byte rowPins[ROWS] = {A6,A8,A7,A9};  //connect to the row pinouts of the keypad
byte colPins[COLS] = {A13,A11,A12,A10}; //connect? toA the column pinouts of the keypad
...
unsigned long ClockInSuccess() {
  character();
   ...
   tft.setTextColor(DARKGREEN);
   //tft.setTextSize(5);
   tft.println("Clock In Succesful");//displays the string
}

unsigned long ClockOutSuccess() {
  character();
   tft.fillScreen(DARKGREEN);
   tft.fillRoundRect(10,70,300,80,5,YELLOW);
   tft.drawRoundRect(10,70,300,80,5,BLACK);
   ...
   tft.setTextColor(DARKGREEN);
   //tft.setTextSize(5);
   tft.println("Clock Out Successful");//displays the string
}
...
void readtag()
{
  ...
  digitalWrite(blue,HIGH); 
    ...
    else if(confirmStoredEntries(scannedTag) == true) 
    {
      ...
      open_sequence();
      mfrc522.PICC_HaltA();
      sendtoESP("{\"cardId\": \"" +scannedTag+ "\",\"attemptedClocking\": \"" +isEnrolled+ "\"}");
      idling();
    }
    else if (confirmStoredEntries(scannedTag) == false){
...
void verify(){
  isEnrolled = "0";
  String httpRequestData = "{\"cardId\": \"" +scannedTag+ "\",\"action\": \"" +action+ "\",\"attemptedClocking\": \"" +isEnrolled+ "\"}"; 
...
void confirmDeleteCode(String codeval){
  codeToDelete = codeval;
  character();
  pos = 12;
  tft.fillScreen(DARKGREEN);
  tft.fillRoundRect(20,20,280,180,10,YELLOW );
  ...
  tft.println(" CODE  ");
  tft.setTextColor(MAROON);
  tft.setCursor(163, 90);
  tft.println(codeval);
  for (uint16_t i=0; i<90; i++){
    tft.drawFastHLine(90+i, 107, 10, DARKGREEN);
  }
  tft.setTextColor(DARKGREEN);
